    ...
    <div style={{ fontFamily: 'Arial, sans-serif', background: '#fff', color: '#18181b' }}>
      <header style={{ borderBottom: '1px solid #e4e4e7', padding: '20px 30px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <a href="#contact" style={{ background: '#18181b', color: '#fff', padding: '10px 18px', borderRadius: 999 }}>Få en samtale</a>
        </div>
      </header>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 30px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 20, marginTop: 30 }}>
          ...
          {process.map((item) => (
            <div key={item.step} style={{ border: '1px solid #e4e4e7', borderRadius: 22, padding: 22, background: '#fafafa' }}>
              <div style={{ color: '#71717a', fontSize: 14 }}>{item.step}</div>
              <h3>{item.title}</h3>
              <p style={{ color: '#52525b', lineHeight: 1.7 }}>{item.text}</p>
            </div>
          ))}
        </div>
      </section>

      <section style={{ background: '#fafafa', padding: '80px 30px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <h2 style={{ fontSize: 34 }}>Hvorfor vælge os</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 20, marginTop: 30 }}>
            {cases.map((item) => (
              <div key={item.title} style={{ border: '1px solid #e4e4e7', borderRadius: 22, padding: 22, background: '#fff' }}>
                <h3>{item.title}</h3>
                <p style={{ color: '#52525b', lineHeight: 1.7 }}>{item.text}</p>
              </div>
            ))}
